...
describe("read queries", () => {
  ...
  it("transaction value is the gross total token output (only token outputs)", async () => {
    // Net "amount sent" isn't derivable on-chain (change goes to fresh
    // addresses), so the list value is the gross total of the token outputs.
    ...
    await db().insert(txOut).values([
      { txId: 0, txHash: "nv1", valueType: "token", amount: "35", locktime: "0", scriptPublicKey: "addr_S", n: 0 },
      { txId: 0, txHash: "nv1", valueType: "token", amount: "3", locktime: "0", scriptPublicKey: "addr_R", n: 1 },
      { txId: 0, txHash: "nv1", valueType: "item", amount: "1", locktime: "0", scriptPublicKey: "addr_R", genesisHash: "g", n: 2 }, // ignored
    ]);
    try {
      const res = await getTransactions(db(), { limit: 100, offset: 0, order: "desc" });
      expect(res.transactions.find((t) => t.hash === "nv1")?.value).toBe("38"); // 35 + 3 tokens; item excluded
    } finally {
